...
public class OnPlusSkillHoverOver : MonoBehaviour,IPointerEnterHandler,IPointerExitHandler
{
    ...
    IEnumerator HoldMouseOver()
    {
        if (GetComponent<Image>().sprite == originSprite)
        {
            ...
            StartCoroutine(TeamSelection.Instance.PlusSkillHoverOverPlus(buttonId));
        }
        else
        {
            yield return new WaitForSeconds(0.6f * Time.timeScale);
            StartCoroutine(TeamSelection.Instance.PlusSkillHoverOverSkill(buttonId));
        }
    }
